...
import {
    ...
    SessionRealCommand,
    SessionRealCommandFromJSON,
    SessionRealCommandToJSON,
    SessionRealResponse,
    SessionRealResponseFromJSON,
    SessionRealResponseToJSON,
...
export interface SessionControllerRealRequest {
    name: string;
    sessionRealCommand?: SessionRealCommand;
}
...
export class SessionApi extends runtime.BaseAPI {
     ...
     * Real
     */
    async sessionControllerRealRaw(requestParameters: SessionControllerRealRequest): Promise<runtime.ApiResponse<SessionRealResponse>> {
        if (requestParameters.name === null || requestParameters.name === undefined) {
            throw new runtime.RequiredError('name','Required parameter requestParameters.name was null or undefined when calling sessionControllerReal.');
        }
        ...
        const response = await this.request({
            path: `/session/{name}/real`.replace(`{${"name"}}`, encodeURIComponent(String(requestParameters.name))),
            method: 'POST',
            headers: headerParameters,
            query: queryParameters,
            body: SessionRealCommandToJSON(requestParameters.sessionRealCommand),
        });

        return new runtime.JSONApiResponse(response, (jsonValue) => SessionRealResponseFromJSON(jsonValue));
    }

    /**
     * Real
     */
    async sessionControllerReal(requestParameters: SessionControllerRealRequest): Promise<SessionRealResponse> {
        const response = await this.sessionControllerRealRaw(requestParameters);
        return await response.value();
    }
