...
import { watchOrderbookDepthSocket } from '@lib/api';
...
import { watchOrderbookDepth } from './watch-orderbook-depth';

jest.mock('@lib/api', () => ({
  ...jest.requireActual('@lib/api'),
  watchOrderbookDepthSocket: jest.fn()
}));

describe(watchOrderbookDepth.name, () => {
...
async function getFixtures() {
  ...
  mockedFunc(watchOrderbookDepthSocket).mockReturnValue(message.asObservable());
  ...
  return {
    ...
    givenOrderbookDepthResolved(instrument: InstrumentSelector) {
      return toArray(act(() => watchOrderbookDepth(instrument, '10@100ms')));
    },
