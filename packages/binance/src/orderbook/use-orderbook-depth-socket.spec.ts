...
import { useOrderbookDepthSocket } from './use-orderbook-depth-socket';

describe(useOrderbookDepthSocket.name, () => {
...
async function getFixtures() {
  ...
  return {
    payload: JSON.parse(
      readFileSync(join(__dirname, 'use-orderbook-depth-socket.payload.json'), 'utf8')
    ),
    ...
    whenOrderbookDepthSocketResolved() {
      return toArray(
        act(() => useOrderbookDepthSocket({ raw: 'BTCUSD' } as any, '10@1000ms'))
      );
    }
  };
}
